...
setwd("~/Downloads") # Mac
setwd("C:/Users/mmeru/Downloads") # Windows
...
forplot <- final %>% group_by(Country) %>% summarize(Days_Since_First_Vaccination=max(Days_Since_First_Vaccination), Vaccination_Rate=last(Vaccination_Rate)) %>% view()
